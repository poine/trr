...
class RaceManager:
    ...
    def set_mode(self, _m, world):
        self.mode = _m
        cbks = [self.enter_staging, self.enter_ready, self.enter_racing, self.enter_finished, self.enter_join_start]
        cbks[self.mode](world)

    def periodic(self, state_est_sub, traffic_light_sub, update_race_mode_cbk):
        cbks = [self.periodic_staging, self.periodic_ready, self.periodic_racing, self.periodic_finished, self.periodic_join_start]
        cbks[self.mode](state_est_sub, traffic_light_sub, update_race_mode_cbk)

    # Race modes behaviour

    # -Staging: we do nothing
    #
    def enter_staging(self, car):
        car.set_guidance_mode(0)      # guidance is idle when staging

    def periodic_staging(self, state_est_sub, traffic_light_sub, update_race_mode_cbk): pass  # and we do nothing
    
    # -Ready: we wait for green light
    #
    def enter_ready(self, car):
        car.set_guidance_mode(1)     # guidance mode is stopped

    def periodic_ready(self, state_est_sub, traffic_light_sub, update_race_mode_cbk):
    ...
    def periodic_racing(self, state_est_sub, traffic_light_sub, update_race_mode_cbk):
    ...
    #
    def enter_finished(self, car):
        car.set_guidance_mode(1) # guidance mode is stopped

    def periodic_finished(self, state_est_sub, traffic_light_sub, update_race_mode_cbk): pass
                
    # -Join Start: we check for approaching start line 
    #
    def enter_join_start(self, car):
        car.set_guidance_mode(2) # driving when going to start

    def periodic_join_start(self, state_est_sub, traffic_light_sub, update_race_mode_cbk, dist_to_stop_at=0.15):
        s_est, v_est, cur_lap, dist_to_start, dist_to_finish = state_est_sub.get()
